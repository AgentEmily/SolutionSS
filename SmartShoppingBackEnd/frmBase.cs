...
namespace SmartShoppingBackEnd
{
    public partial class frmBase : Form
    {
        ...
        public virtual void btnFirst_Click(object sender, EventArgs e)//首筆
        {
        }

        public virtual void btnPrior_Click(object sender, EventArgs e)//前筆
        {
        }

        public virtual void btnNext_Click(object sender, EventArgs e)//次筆
        {
        }

        public virtual void btnLast_Click(object sender, EventArgs e)//末筆
        {
        }

        public virtual void btnAddNew_Click(object sender, EventArgs e)//新增
        {
        }

        public virtual void btnEdit_Click(object sender, EventArgs e)//修改
        {
        }

        public virtual void btnDelete_Click(object sender, EventArgs e)//刪除
        {
        }

        public virtual void btnCancelEdit_Click(object sender, EventArgs e)//取消
        {
        }

        public virtual void btnSaveChange_Click(object sender, EventArgs e)//儲存
        {
        }
    ...
    //    string FProgramID;
    //    //建立存取共用類別的變數
    //    SmartShoppingFunction SSFunction;
    ...
    //        {
    //            //取得客戶資料表所有的記錄
    //            //var qry = context.Members.Execute(MergeOption.OverwriteChanges);
    //            var qry = from p in context.Members select p;
    //            Debug.WriteLine(qry.ToString());

    //            //將取得的結果指派給BindingSource控制項的DataSource
    //            //MembersBindingSource.DataSource = qry;
    //            FBindingSource.DataSource = qry.ToList();
    //        }
    ...
    //    {
    //        this.FBindingSource.DataSource = typeof(SmartShoppingBackEnd.Members);
    ...
    //          //      //              "FROM SmartShoppingEntities." + FTableName + " AS T ";

    //          //      string ESQL = "SELECT MIN(T." + FIDFieldName + ") AS ID " +
    //          //"FROM SmartShoppingEntities." + FTableName + " AS T ";

    //          //      DbDataRecord rec = new ObjectQuery<DbDataRecord>(ESQL, ((IObjectContextAdapter)context).ObjectContext).FirstOrDefault();
    //          //      if (rec[0] != null)
    //          //      {
    ...
    //    {
    //        if (! SSFunction.CheckAuthority(FProgramID, 1))
    //        {
    ...
    //    {
    //        if (!SSFunction.CheckAuthority(FProgramID, 2))
    //        {
    ...
    //    {
    //        if (!SSFunction.CheckAuthority(FProgramID, 3))
    //        {
    ...
    //                    rec = new ObjectQuery<DbDataRecord>(ESQL, ((IObjectContextAdapter)context).ObjectContext).FirstOrDefault();
    //                    if (rec != null)
    //                    {
    //                        PrimaryID = (string)(rec[0]);
    //                        BeforeLoad();
    ...
    //    {
    //        if (!SSFunction.CheckAuthority(FProgramID, 4))
    //        {
    ...
    //    {
    //        // TODO:  這行程式碼會將資料載入 'smartShoppingDataSet.Members' 資料表。您可以視需要進行移動或移除。
    //        //this.membersTableAdapter.Fill(this.smartShoppingDataSet.Members);
    //        FBindingSource = new BindingSource();
    //        FBindingSource.PositionChanged += 
    //            new EventHandler(this.FBindingSource_PositionChanged);
    //        //建立共用函數類別
    //        SSFunction = new SmartShoppingFunction();
    //        //預設狀態為瀏覽模式
    //        TBStatus = 0;
    //        //所有按鈕的狀態
    //        SetButtons();
